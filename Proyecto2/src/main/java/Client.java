...
public class Client {
	...
	public Client(String dirName, String fullDirName, String serverIP, int port) {
		...
		System.out.println("Directorio a sincronizar: " + dirName);
	...
	public void runClient() throws Exception {
		...
		oos = new ObjectOutputStream(sock.getOutputStream()); //enviando el nombre del directorio al servidor
		...
		if(dirName.equalsIgnoreCase("-ls")) {
			...
			System.out.println("Los directorios disponibles son: ");
			for (int x = 0; x < directories.size(); x++) {
				System.out.print(directories.elementAt(x) + " ");
			}
			System.out.println();
			System.out.println();
			System.out.println("Para sincronizar uno de estos directorios por favor cree el que usted desee sincronizar y ejecute de nuevo este comando con el directorio seleccionado.");
		} else {
			System.out.print("Sincronizando");
			// Recibe si el directorio existe
			Boolean fExists = (Boolean) ois.readObject();

			File baseDir = new File(fullDirName); // omite el directorio base si ya ha sido seteado en el servidor
			...
			System.out.println();
			System.out.println("Finalizando sincronizador.");
		}
		oos.close();
		ois.close();
		sock.close();
	...
	private static void visitAllDirsAndFiles(File dir) throws Exception{
		...
		if(dir.isDirectory()) {
			...
			Integer updateToServer = (Integer) ois.readObject(); //if true actualiza el server, else update from server

			if (updateToServer == 1) {  // enviando archivo al servidor
				sendFile(dir);

				ois.readObject(); // validando que el server reciba el archivo

			} else if (updateToServer == 0) { //actualizando archivo del server.
				dir.delete(); // primero se borra el archivo actual
